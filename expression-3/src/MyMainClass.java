...
/*
0
+(-(-x**+1+cos(x)**0*x)*x**2+sin(x)**1*
(-x*x*x)++4*(-x*x--4*sin(x)**0*sin(x)**2-sin(x)**0*x))*
(-(-x-x)*(+x-x+x)*(+x))*(+x**1)+x**2+(+(-cos(x)**+0*x-cos(x)**+1*x**+1+x*x*x**0)*
(+cos(x)**+0+x))*sin(x)**1
 */
...
1
f(x,z)=x**2+sin(z*(x+1))
sum(i,1,10,f(sin(x**2),cos(sin(x+--1))**2)*x*(1)*(((x))))
 */

...
/*    public Expression parseExpression() {  // only return expression
    ...
    public Expression parseSum() {
        holder.nextParam();
        holder.nextParam();
        BigInteger begin = new BigInteger(holder.getCurToken());
        holder.nextParam();
        BigInteger e = new BigInteger(holder.getCurToken()).add(BigInteger.ONE);
        holder.nextParam();
        String expString = "".concat(holder.getCurToken().replaceAll("loopVar",
                                                                     String.valueOf(begin)));
        expString = "0";
        for (BigInteger i = begin;i.compareTo(e) < 0;i = i.add(BigInteger.ONE)) {
